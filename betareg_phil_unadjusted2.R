...
beta_reg <- function(meth.data, covariates, reg.results.table, hist){
  ...
  for (i in 1:N){
    ...
    a.freq <- calc_allele_freq(reg.table[4])
    reg.results.table$Prevalence[i] <- a.freq
    
    #Calculate allele freq with SNP that I'm testing or entire SNP data?
    ...
    SNP <- unlist(reg.table[4], use.names=FALSE)
    ...
    if (all(Platform.Indicator == Platform.Indicator[1]) == TRUE){
      ...
      model <- betareg(Beta_Value ~ SNP + Platform.Indicator, link="logit")
      reg.results.table$Estimate[i] <- model$coefficients$mean[2]
      reg.results.table$P.Value[i] <- summary(model)$coefficients$mean[2,4]
...
write.csv(x, "rerun_unadjusted_reg_results.csv", row.names=FALSE)
