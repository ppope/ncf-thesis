...
reg.results <- read.csv("D:/Phil/betareg/output/total_reg_results_unadjusted.csv", header=TRUE)
reg.results[,3] <- sub("\\.", "-", reg.results[,3])
names(reg.results)[c(1,4)] <- c("SNP.ID", "Meth.Probe.Name")
...
write.csv(reg.results, "D:/Phil/betareg/output/total_reg_results_unadjusted_postprocessed.csv", row.names=FALSE)