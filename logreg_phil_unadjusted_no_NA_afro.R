...
M <- length(sig.probes)
case.control.dfrm <- data.frame("Meth.Probe" = sig.probes, "Controls"=rep(0,M), "Cases" = rep(0,M))
meth.data <- ADC.meth.data.dicho
SNP.data <- ADC.SNP.data
reg.results.table <- results.table
hist <- "ADC"

logistic_reg <- function(meth.data, SNP.data, reg.results.table, hist, case.control.table){
...
ADC.logreg.results1 <- logistic_reg(dichotomize_meth_data(ADC.meth.data, meth.cutoffs1), ADC.SNP.data, results.table, "ADC")
SCC.logreg.results1 <- logistic_reg(dichotomize_meth_data(SCC.meth.data, meth.cutoffs1), SCC.SNP.data, results.table, "SCC")

ADC.logreg.results2 <- logistic_reg(dichotomize_meth_data(ADC.meth.data, meth.cutoffs2), ADC.SNP.data, results.table, "ADC")
SCC.logreg.results2 <- logistic_reg(dichotomize_meth_data(SCC.meth.data, meth.cutoffs2), SCC.SNP.data, results.table, "SCC")

ADC.logreg.results3 <- logistic_reg(dichotomize_meth_data(ADC.meth.data, ADC.meth.cutoffs3), ADC.SNP.data, results.table, "ADC")
SCC.logreg.results3 <- logistic_reg(dichotomize_meth_data(SCC.meth.data, SCC.meth.cutoffs3), SCC.SNP.data, results.table, "SCC")

setwd("D:/Phil/betareg/output/")
x <- rbind(ADC.logreg.results1, SCC.logreg.results1)
write.csv(x, "unadjusted_logreg_results_cutoff1.csv", row.names=FALSE)

y <- rbind(ADC.logreg.results2, SCC.logreg.results2)
write.csv(y, "unadjusted_logreg_results_0.25_cutoff.csv", row.names=FALSE)

z <- rbind(ADC.logreg.results3, SCC.logreg.results3)
write.csv(z, "unadjusted_logreg_results_median_cutoff.csv", row.names=FALSE)
